...
- デスクトップと Android のレスポンシブ UI を全面的に改善。
- メンバーの暗号化アバター、権限付き背景画像、画像スタンプを追加。
- サーバー所有者が `mods/` ディレクトリへ安全なデータ専用 JSON モジュールを配置し、権限付きスラッシュコマンドを提供可能。
- Android 10 以降はサーバー参加と全クライアント・管理機能に対応。ただしサーバー作成・ホスト機能は非搭載。

詳細は [Android Client](docs/ANDROID.md) と [Server Mods](docs/MODS.md) を参照してください。
